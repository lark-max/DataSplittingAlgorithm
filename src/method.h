...
namespace method{
	...
	public:
		...
		Method();
		Method(const double& a, const double& b, som::matrix<double>& data, som::matrix<double>& inidata);
		~Method();
		// basic methods	
		void	StructCopy(std::unique_ptr<som::SOMProcess>& ptr){afterSOM = std::move(ptr);}
		void	Set_vector();
