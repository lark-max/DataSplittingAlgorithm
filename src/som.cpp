...
void SOMProcess::Train(const double& learnRate, const int neighborSize, const int Epochs)
{
	...
	for (; epoch < Epochs; ++epoch)
	{
		...
		for (int i=0; i < totalData; ++i)
		{
			...
			learningRate = decayFunction * learnRate;
			neighbourhoodSize = (int)(decayFunction * (double)neighborSize + 0.5);
...
void  SOMProcess::BeforeTrainProcess(matrix<double>& Data)
{
	...
	dimensions = InputData.columns();

	neuronNumber = (int)(2 * sqrt(totalData) + 0.5);
	neuronColumn = (int)(sqrt(neuronNumber / 1.6) + 0.5);
...
void	SOMProcess::GetActiveNeuron(const vector<double>& Data_I)
{
	...
	for (int i = 0; i < neuronRow; ++i)
		for (int j = 0; j < neuronColumn; ++j)
		{
			double distance_ = Getdistance_((*this)[i][j].W, Data_I) - consicience * (1.0 / neuronNumber - (*this)[i][j]._bias);
...
{
	if (activeRow < neighbourhoodSize)
		neighbourhoodLowerRowIndex_ = 0;
	else
		neighbourhoodLowerRowIndex_ = activeRow - neighbourhoodSize;

	if (neuronRow - 1 < activeRow + neighbourhoodSize)
		neighbourhoodUpperRowIndex_ = neuronRow - 1;
	else
		neighbourhoodUpperRowIndex_ = activeRow + neighbourhoodSize;

	if (activeColumn < neighbourhoodSize)
		neighbourhoodLowerColumnIndex_ = 0;
	else
		neighbourhoodLowerColumnIndex_ = activeColumn - neighbourhoodSize;

	if (neuronColumn - 1 < activeColumn + neighbourhoodSize)
		neighbourhoodUpperColumnIndex_ = neuronColumn - 1;
	else
		neighbourhoodUpperColumnIndex_ = activeColumn + neighbourhoodSize;
}
...
void	SOMProcess::UpdateWeight(const vector<double>& Data_I)
{
	for (int i = neighbourhoodLowerRowIndex_; i < neighbourhoodUpperRowIndex_; ++i)
		for (int j = neighbourhoodLowerColumnIndex_; j < neighbourhoodUpperColumnIndex_; ++j)
		{
			...
			double h = exp(-(d * d / (2.0 * neighbourhoodSize * neighbourhoodSize)));
			for (int i1 = 0; i1 < dimensions; ++i1)
				(*this)[i][j].W[i1] += h * learningRate * (Data_I[i1] - (*this)[i][j].W[i1]);
