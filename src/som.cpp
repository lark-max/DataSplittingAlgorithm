...
void	SOMProcess::GetActiveNeuron(const vector<double>& Data_I)
{
	...
	double Active_Distance = Getdistance((*this)[0][0].m_weight, Data_I);
	for (int i = 0; i < m_neuronRow; ++i)
		for (int j = 0; j < m_neuronColumn; ++j)
		{
			double distance_ = Getdistance((*this)[i][j].m_weight, Data_I) - m_consicience * (1.0 / m_neuronNumber - (*this)[i][j].m_bias);
...
double	SOMProcess::Getdistance(const vector<double>& m, const vector<double>& n)
{
	...
	if (m.size() != n.size())
	{
		...
		outCerr << "[double	SOMProcess::Getdistance()]: Fatal error: Dimenson mismatch!" << std::endl;
		outCerr.close();
		exit(1);
	}
	else
